...
import { loadStakeDaoInOut, loadStakeDaoPool } from '../../core/stake-dao/stake-dao.actions';
...
export class StakeDaoPoolComponent implements OnInit {
  ...
  ngOnInit() {
    this.stakeDaoPoolState$ = this.store.select(state => state.stakeDao.stakeDaoPoolsState[this.id])
    this.stakeDaoPoolState$.pipe(take(1)).subscribe(v => {
      if (v.stakeDaoPoolInfo.contract.length != 2 && v.loading == true) {
        this.store.dispatch(loadStakeDaoPool({ poolId: this.id, contract: v.stakeDaoPoolInfo.contract }));
      }
    });
  }

  loadInOut(s : StakeDaoPoolState){
    this.store.dispatch(loadStakeDaoInOut({contract : s.stakeDaoPoolInfo.contract}));
    this.dialog.open(StakeDaoInOutFlowComponent,{data:{title : s.sdtoken}}
      )
  }
